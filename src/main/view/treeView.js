...
class TreeView {
    ...
    // an action) - draws all of them during the undecided reveal/spinning-arrow phases (matching
    // SimulationAnimator's reveal flash), just the one chosen next pathId during 'highlight', or
    // nothing at all during 'transition'/'idle' (current has already advanced past this decision,
    // or is at rest waiting for the next one - nothing has been revealed for its own children
    // yet). Unlike Graph view, this does NOT reuse simState.isNodeVisible/isEdgeVisible for the
    // frontier: those are tracked globally per real node id and never cleared, which is
    // unambiguous in Graph view (one real id = one canvas position) but not in Tree view, where
    // the same real id can occupy multiple
    ...
    _drawTraceReveal(tree, simState) {
        ...
        // Frontier fan: SimulationAnimator.animateTransition() runs `advance()` (which moves
        // simState.currentIndex/currentNode) BEFORE setPhase('transition', ...) - see its Phase 4
        // then Phase 5 - so by the time phase is 'transition' or 'idle', `current` here (=
        // pathMap.get(pathIds[ci])) is already the node the decision just landed ON, not the node
        // that made the decision; its own children (the NEXT decision) haven't started being
        // revealed at all yet, so nothing should show for them. 'highlight' fires BEFORE
        // advance(), so during 'highlight' `current` is still the pre-advance node and its
        // frontier has genuinely just been decided - that's when exactly one child (the chosen
        // one) should show, matching the travel ball moving toward it. 'reveal'/
        // 'state_spinning_arrow'/'spinning_arrow' are the genuinely undecided phases (matching
        // SimulationAnimator's reveal-then-narrow flash) where ALL of current's real children
        // show. The chosen child is identified by pathIds[ci+1] - known directly from our own
        // trace->pathId mapping (Task 3), not from simState's global per-real-id visibility
        // flags. This deliberately does NOT use simState.isNodeVisible/isEdgeVisible for the
        // frontier: those flags are sticky per real id and never cleared, so a cyclic destination
        // whose real id was already revealed earlier in the trace (e.g. back to the start state)
        // would read as "visible" for a brand new tree-position occurrence regardless of whether
        // this specific decision has actually been revealed - producing a spurious preview.
        const current = pathMap.get(pathIds[ci]);
        const FAN_ALL_PHASES = new Set(['reveal', 'state_spinning_arrow', 'spinning_arrow']);
        const chosenNextPathId = (ci + 1 < pathIds.length) ? pathIds[ci + 1] : null;
        const showAllOfFan = FAN_ALL_PHASES.has(simState.phase);
        const showOnlyChosen = simState.phase === 'highlight';

        if (current) {
            current.children.forEach(child => {
                const showThisChild = showAllOfFan || (showOnlyChosen && child.pathId === chosenNextPathId);
